...
import au.id.jazzy.play.geojson.{Feature, FeatureCollection, LatLng, Point, Polygon}
...
//  def props(email: String, upstream: ActorRef): Props =
//    Props(new ClientConnection(, data))
  ...
  object ClientEvent {
     ...
     */
    implicit val transformer = MessageFlowTransformer.jsonMessageFlowTransformer[ClientEvent, ClientEvent]
...
class ClientConnection @Inject()(
  ...
  def receive = {
    ...
    case UserMoved(point) =>
      context.actorSelection("/user/regionManagerClient") ! UserPosition(email, System.currentTimeMillis(), point.coordinates)
